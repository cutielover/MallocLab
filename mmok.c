...
void* realloc(void* oldptr, size_t size) {
  ...
  if(dval <= 0){
  	if(dval > -16)
  		...
  		PUT_PACK(GET_HEADER(oldptr),asize,BTAG_KEEP,1);
  		
  		void *np = GET_NEXT_BLOCK(oldptr);
  		PUT_PACK(GET_HEADER(np),-dval,BTAG_ALLOC,0);
  		PUT_PACK(GET_FOOTER(np),-dval,BTAG_ALLOC,0);
  		PUT_FREE_BTAG(GET_HEADER(GET_NEXT_BLOCK(np)));
  		seglist_insert(np,-dval);
  		return oldptr;
  	}
  }
